(ns iceye.main
  "Kotodama WASM entrypoint for the iceye clean-room actor (L4) — Clojure port.
  ...
  facts (`iceye.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"iceye.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iceye-compat" :tier tier :entities entities} 200])
